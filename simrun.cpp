...
sim_results simrun( const sim_parameters par, const string dir_init )
{
    ...
    if ( par.system_type == 1 ) {
        model = new IsingModel1d( par.N, par.periodic, par.J, par.B, par.T, par.use_fsize_correction, dir );
    } else if ( par.system_type == 2 ) {
        model = new IsingModel2d( par.N, par.periodic, par.J, par.B, par.T, par.use_fsize_correction, dir );
    } else if ( par.system_type == 3 ) {
        model = new IsingModel2dWolff( par.N, par.periodic, par.J, par.T, par.use_fsize_correction, dir );
    } else if ( par.system_type == 4 ) {
        model = new IsingModel2dDipole( par.N, par.periodic, par.J, par.g, par.B, par.T, dir );
    } else if ( par.system_type == 5 ) {
        model = new Ising2dDipSqr( par.N, par.periodic, par.J, par.g, par.B, par.T, par.use_fsize_correction, dir );
    } else if ( par.system_type == 6 ) {
        model = new Ising2dDipHC( par.N, par.J, par.g, par.B, par.T, par.use_fsize_correction, dir );
    } else {
    ...
    run_log
            << ( ( sizeof( m_memlog ) + m_memlog.capacity() * sizeof( float )
                   + sizeof( binres ) + binres.capacity() * sizeof( bin_results ) ) / 1024 ) + 1
    ...
    for ( unsigned int bin = 0; bin < par.bins; bin++ ) {
        ...
        for ( unsigned int sample = 0; sample < par.binwidth; sample++ ) {
            ...
            if ( ( par.take_images != 0 ) && ( ( bin * par.binwidth + sample ) % par.take_images == 0 ) ) {
    ...
    for ( unsigned int bsset = 0; bsset < par.bins; bsset++ ) {
        ...
        double bsset_c = spin_count / par.T / par.T * ( bsset_h2 - bsset_h * bsset_h );
    ...
    if ( par.calc_autocorr ) {
        ...
        for ( unsigned int d = 0; d < par.binwidth; d++ ) {
            ...
            acout_log << d* par.intersweeps << ' ' << ( product - m * m ) / norm << endl;
        }
        acout_log.close();
    ...
    run_log.precision( numeric_limits<float>::digits10 + 1 ); // set number output format etc.
    ...
    tmp << "system_type = " << par.system_type
        ...
        << ", J = " << par.J << ", g = " << par.g << ", B = " << par.B << ", T = " << par.T;
    int plot_size = ( par.bins * par.binwidth < 1600 ) ? 1600 : min( ( uint )10000, par.bins * par.binwidth );
    run_plot <<
    ...
    run_plot <<
             "	plot 'm.log' with lines ls 2 title 'magnetization (" << m << " +- "	<< sigma3_m << ")' \n\
	unset arrow\n\
	set arrow from graph 0,first " << h << " to graph 1,first " << h << " ls -1 lw 2 nohead" << endl;
    for ( unsigned int bin = 0; bin < par.bins; bin++ ) {
        double sep = double( bin ) / double( par.bins );
        run_plot <<
                 "		set arrow from graph " << sep << ",0 to graph " << sep << ",1 nohead\n\
    ...
    run_plot <<
             "	plot 'h.log' with lines ls 1 title 'energy per spin (" << h << " +- " << sigma3_h << ")' \n";
    tmp.str( "" );
    run_plot.close();
    ...
    if ( par.calc_autocorr ) {
		...
		set arrow from first " << tau << ", graph 0 to first " << tau << ", graph 1 nohead\n\
    ...
    if ( par.calc_sscorr ) {
        ...
        sscorr_plot.close();
        if ( par.run_plot ) {
            if ( system( ( "cd " + dir + " ; gnuplot sscorr_plot.gnu" ).c_str() ) != 0 ) {
