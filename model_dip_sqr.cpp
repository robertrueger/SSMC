...
Ising2dDipSqr::Ising2dDipSqr( const unsigned int& size, const bool& periodic,
    ...
    : SystemModel( true, B, T, cwd ), J( J ), g( g ), N( size* size ), size( size ),
      fsize_correction_mode( fsize_correction_mode ), fsize_ordered_phase( false ),
      k_range( 2 * M_PI ), k_points( 201 ), sf_measurements( 0 )
{
    // wrong use of the model?
    if ( !periodic ) {
        cout <<	"Model: WARNING - Non-periodic boundary conditions are not supported!" << endl;
    }
    if ( size % 2 != 0 ) {
    ...
    // set up an array for the precalculated interaction coefficients: effint[dl][dc]
    ...
    if ( !effint_log.is_open() ) {
        cout << "ERROR while opening effective interaction log file in " << cwd << endl;
        exit( 1 );
    }
    ...
    for ( unsigned int dl = 0; dl < ( size / 2 ) + 1; ++dl ) {
        for ( unsigned int dc = 0; dc < dl + 1; ++dc ) {
            ...
            effint_log << "effint[" << dl << "][" << dc << "] = " << effint[dl][dc] << endl;
        }
    }
...
{
    // custom destructor, because we need to write the structure factor to the logfile first
    ...
    for ( unsigned int line = 0; line < k_points; line++ ) {
        for ( unsigned col = 0; col < k_points; col++ ) {
            ...
            sf_log << kx << ' ' << ky << ' ' << sf_sum[line][col] / sf_measurements / N << endl;
        }
    }
...
double Ising2dDipSqr::calc_effint_dipdip( const int& dl_seed, const int& dc_seed, const int& system_clones )
{
    ...
    double result = 0;
    //uint count = 0;
    for ( long int dl = dl_seed - system_clones * size; dl < ( system_clones + 1 )*size; dl += size ) {
        for ( long int dc = dc_seed - system_clones * size; dc < ( system_clones + 1 )*size; dc += size ) {
            double r = sqrt( dl * dl + dc * dc );
            if ( r == 0 ) {
                continue;    // no interaction with itself ...
            }
            result += g / ( r * r * r );
            //cout << dl << ' ' << dc << ' ' << ' ' << dl*dl+dc*dc << ' ' << r << ' ' << result << endl;
            //count++;
            //if (count == 100) exit(0);
...
void Ising2dDipSqr::metropolis_singleflip()
{
    ...
    for ( int line = 0; line < size; line++ ) {
        for ( int col = 0; col < size; col++ ) {
            ...
            deltaH += - 2 * effint[dl][dc] * ( spin[flip_line][flip_col] * spin[line][col] );
        }
    }
